...
namespace PerpustakaanAppMVC.View
{
    public partial class FrmMahasiswa : Form
    {
        ...
        private void TampilkanDataKeListView (List<Mahasiswa> dataMahasiswa)
        {
            lvwMahasiswa.Items.Clear();

            foreach (var mhs in dataMahasiswa)
            {
                var noUrut = lvwMahasiswa.Items.Count + 1;
                var item = new ListViewItem(noUrut.ToString());
                item.SubItems.Add(mhs.Npm);
        ...
        {
            string namaCari = txtNama.Text.Trim().ToLower();

            // Filter mahasiswa berdasarkan nama
            var filteredMahasiswa = listOfMahasiswa.Where(mhs => mhs.Nama.ToLower().Contains(namaCari)).ToList();

            TampilkanDataKeListView(filteredMahasiswa);
        }
    }
}
